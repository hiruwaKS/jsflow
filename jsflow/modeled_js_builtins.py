...
def setup_object_and_function(G: Graph):
    ...
    G.add_blank_func_as_prop('toLocaleString', object_prototype, object_p_to_string_simp)
...
def array_p_for_each_static_new(G: Graph, caller_ast, extra, array: NodeHandleResult, callback=NodeHandleResult(), this=NodeHandleResult()):
    ...
    opgen.call_function(G, callback.obj_nodes, args=args,
        extra=new_extra, call_ast=caller_ast, func_name=callback.name)
    return NodeHandleResult()


def array_p_push(G: Graph, caller_ast, extra, arrays: NodeHandleResult, *tobe_added_objs: NodeHandleResult):
    used_objs = set()
    if extra.branches.get_last_choice_tag() and len(arrays.obj_nodes) < G.array_obj_node_limit:
...
def array_p_join_1(G: Graph, caller_ast, extra, arrays: NodeHandleResult, seps=NodeHandleResult(values=[','])):
    returned_objs = []
    used_objs = set()
        ...
        if length != wildcard:
            for i in range(start, length):
                returns = opgen.call_function(G, callback.obj_nodes,
                    ...
                    extra=extra, call_ast=caller_ast)[0]
                accumulator = returns
        else:
            for name_node in G.get_prop_name_nodes(arr):
                name = G.get_node_attr(name_node).get('name')
...
def object_keys(G: Graph, caller_ast, extra, _, arg: NodeHandleResult, for_array=False):
    ...
    for obj in arg.obj_nodes:
        ...
        i = 0
        # wildcard object
        if G.get_node_attr(obj).get('type') in ['array', 'object', wildcard] and \
...
def object_p_to_string_simp(G: Graph, caller_ast, extra,
        this: NodeHandleResult, *args):
    string = G.add_obj_node(caller_ast, 'string', '[object Object]')
                    ...
                    if sv is None or ssv is None or sv == wildcard or ssv == wildcard:
                        ...
                        def python_cb(m):
                            ...
                            cb_result, _ = \
                                ...
                                args=args, extra=extra, call_ast=caller_ast)
...
def promise_constructor(G: Graph, call_ast, extra, _, executor=NodeHandleResult()):
    ...
    dummy_stmt_ast = G.get_child_nodes(
        G.get_child_nodes(G.get_obj_def_ast_node(G.promise_cons), child_type='AST_STMT_LIST')[0],
...
def promise_p_then(G: Graph, call_ast, extra, this, on_fulfilled=NodeHandleResult(), on_rejected=NodeHandleResult()):
