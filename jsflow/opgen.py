...
def instantiate_obj(G, exp_ast_node, constructor_decl, branches=None):
    ...
    returned_objs, _ = jsflow_function(G, constructor_decl, branches=branches,
        ...
        for i, case in enumerate(cases):
            ...
            if G.get_node_attr(test).get('type') == 'NULL': # default
                if default_is_deterministic or G.single_branch:
                    ...
                    # not deterministic
                    G.last_stmts = [case]
                    jsflow_block(G, body, G.cur_scope, branches+[branch_tag],
                                  block_scope=False)
                    last_stmts.extend(G.last_stmts)
                continue
            if G.first_pass:
...
def jsflow_function(G, func_ast, branches=None, block_scope=True,
    caller_ast=None):
    """
    ...
    for child in G.get_child_nodes(func_ast, child_type='AST_STMT_LIST'):
        ...
        returned_objs, used_objs = jsflow_block(G, child,
...
def jsflow_block(G, ast_node, parent_scope=None, branches=None,
...
def jsflow_block_init(G, ast_node, parent_scope=None, branches=None,
    block_scope=True, decl_var=False):
    """
    Analyze a block by running its statements one by one.
    A block is a BlockStatement in JavaScript,
...
def jsflow_block_finalize(G, ast_node=None, parent_scope=None, branches=None,
    block_scope=True, decl_var=False):
