...
def get_module(G, name):
    if name in modeled_modules:
        if name in G.builtin_modules:
            ...
            logger.log(ATTENTION, "Setting up built-in module {}...".format(name))
...
def read_file_sync(
    ...
    for path in paths:
        ...
        abs_path = os.path.normpath(
            os.path.join(G.get_cur_file_path(), "..", str(path))
        )
            ...
            def returned_func(G: Graph, caller_ast, extra, this, *args):
                def executor(
                    ...
                    result, _ = opgen.call_function(
                        G, [func_obj], args, this, extra, caller_ast
                    )
                    opgen.call_function(
                        G,
                        to_obj_nodes(G, on_fulfilled, caller_ast),
                        args=[result],
                ...
                G.set_node_attr(
                    promise, ("executors", to_obj_nodes(G, executor, caller_ast))
                )
        ...
        if _format is None or _format == wildcard or type(_format) != str:
            ...
            def dfs1(i, buffer, sources):
                ...
                if i >= len(args):
                    ...
                    opgen.add_contributes_to(
                        G, sources, result_obj, operation="string_concat"
                    )
                ...
                for obj in objs:
                    ...
                    if buffer == wildcard or value == wildcard:
                        ...
                        dfs1(i + 1, buffer + value, sources + [space_obj(), obj])
...
modeled_modules = {"fs": setup_fs, "util": setup_util, "path": setup_path}
