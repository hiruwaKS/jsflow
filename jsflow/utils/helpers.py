...
def convert_prop_names_to_wildcard(
    G: Graph, obj, exclude_length=False, exclude_proto=True
):
...
def to_python_array(G: Graph, array_obj, value=False):
    ...
    for name_node in G.get_prop_name_nodes(array_obj):
        ...
        for e in G.get_out_edges(name_node, edge_type="NAME_TO_OBJ"):
            if value:
                elements[i].append(val_to_str(G.get_node_attr(e[1]).get("code")))
            else:
...
def to_og_array(G: Graph, elements, edge_data, ast_node=None):
    ...
    for i, elem in enumerate(elements):
        ...
        for j, obj in enumerate(elem):
            G.add_edge(name_node, obj, {"type:TYPE": "NAME_TO_OBJ", **edge_data[i][j]})
    G.add_obj_as_prop(
        prop_name="length",
        ast_node=ast_node,
        js_type="number",
...
def is_wildcard_obj(G, obj):
    ...
    return (
        ...
        or (attrs.get("type") in ["number", "string"] and attrs.get("code") == wildcard)
