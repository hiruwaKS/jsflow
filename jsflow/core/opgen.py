...
# Initialize with console output to avoid creating run_log.log in root directory
logger = create_logger("main_logger", output_type="console")
eval_logger = None  # Will be initialized in log_cf with proper log directory
...
def add_edges_between_funcs(G):
    ...
    for call_edge in call_edges:
        ...
        ln2 = G.get_node_attr(list(G.get_in_edges(entry_edge[1]))[0][0]).get(
            "lineno:int"
        )
        ...
        logger.info(
            ...
            + "Add CFG edge"
            + sty.rs.all
            + " {} -> {} (Line {} -> {})".format(CPG_caller_id, entry_edge[1], ln1, ln2)
        )
        ...
        added_edge_list.append(
            (CPG_caller_id, entry_edge[1], {"type:TYPE": "FLOWS_TO"})
        )
        ...
        for idx in range(min(len(callee_paras), len(caller_para_names))):
            ...
            logger.info(
                sty.ef.inverse
                + sty.fg.li_magenta
                + "Add INTER_FUNC_REACHES"
                + sty.rs.all
                + " {} -> {} (Line {} -> Line {})".format(
                    CPG_caller_id, callee_paras[idx], ln1, ln2
                )
            )
            assert (
                CPG_caller_id != None
            ), "Failed to add CFG edge. CPG_caller_id is None."
            assert (
                callee_paras[idx] != None
            ), f"Failed to add CFG edge. callee_paras[{idx}] is None."
            added_edge_list.append(
                (
                    CPG_caller_id,
                    callee_paras[idx],
                    {
                        "type:TYPE": "INTER_FUNC_REACHES",
                        "var": str(caller_para_names[idx]),
                    },
                )
            )
                ...
                for stmt in G.get_child_nodes(child, "PARENT_OF"):
                    if G.get_node_attr(stmt)["type"] == "AST_RETURN":
                        ...
                        logger.info(
                            sty.ef.inverse
                            + sty.fg.li_magenta
                            + "Add return value data flow"
                            + sty.rs.all
                            + " {} -> {} (Line {} -> Line {})".format(
                                stmt, CPG_caller_id, ln1, ln2
                            )
                        )
                        assert (
                            stmt != None
                        ), "Failed to add CFG edge. Statement ID is None."
                        assert (
                            CPG_caller_id != None
                        ), "Failed to add CFG edge. CPG_caller_id is None."
                        added_edge_list.append(
                            (stmt, CPG_caller_id, {"type:TYPE": "FLOWS_TO"})
                        )
...
def register_func(G, node_id):
    ...
    logger.info(
        sty.ef.b
        + sty.fg.green
        + "REGISTER {} to {}".format(node_id, parent_func_nodeid)
        + sty.rs.all
    )


def find_prop_legacy(
    G,
    parent_objs,
    prop_name,
    branches=None,
    ...
    if in_proto:
        logger.debug(
            'Cannot find "direct" property, going into __proto__ ' f"{parent_objs}..."
        )
    ...
    for parent_obj in parent_objs:
        if (
            prop_name == wildcard
            and not is_wildcard_obj(G, parent_obj)
            and not G.check_proto_pollution
        ):
            continue
        ...
        if prop_name_node is not None:
            ...
            if __proto__name_node is not None:
                ...
                if __proto__obj_nodes:
                    __name_nodes, __obj_nodes = find_prop_legacy(
                        G,
                        __proto__obj_nodes,
                        prop_name,
                        branches,
        ...
        if (
            not name_node_found
            and not in_proto
            and prop_name != wildcard
            and side != "left"
        ):
            # try wildcard (*)
            r1, r2 = find_prop_legacy(
                G,
                [parent_obj],
                wildcard,
                branches,
        ...
        if not name_node_found and not in_proto:
            ...
            if is_wildcard_obj(G, parent_obj) and side != "left":
                ...
                added_obj = G.add_obj_to_name_node(
                    added_name_node,
                ...
                logger.debug(
                    "{} is a wildcard object, creating a wildcard"
                    " object {} for its properties".format(parent_obj, added_obj)
                )
                ...
                if side == "right":
                    ...
                    #                     ('for_tags', prop_name_for_tags))
                    logger.debug(
                        f"{sty.ef.b}Add prop name node{sty.rs.all} "
...
def find_prop(
    ...
    def add_arg_paths(obj_node, prop_name, prop_name_sources=[]):
        ...
        if (
            len(prop_name_sources) == 1  # doesn't have to be wildcard
            # don't support multiple sources (from immediate values instead of obj nodes)
            and prop_name_sources[0] != obj_node
        ):  # no self reference
            prop_name_access_paths = G.get_node_attr(prop_name_sources[0]).get(
                "arg_paths", set()
            )
    ...
    for parent_obj in parent_objs:
        ...
        parent_arg_paths = set(
            islice(G.get_node_attr(parent_obj).get("arg_paths", set()), 100)
        )
        ...
        if prop_name == wildcard:
            if G.first_pass:
                ...
                not is_wildcard_obj(G, parent_obj)
                and not G.check_proto_pollution
                and not G.check_ipt
            ):
                continue
        ...
        # If name node is not found, search the property under __proto__.
        # Note that we cannot search "__proto__" under __proto__.
        elif prop_name != "__proto__" and prop_name != wildcard and side != "left":
            __proto__name_node = G.get_prop_name_node("__proto__", parent_obj)
            if __proto__name_node is not None:
                __proto__obj_nodes = G.get_objs_by_name_node(
                    __proto__name_node, branches
                ...
                if parent_obj in __proto__obj_nodes:
                    logger.error(
                        f"__proto__ {__proto__obj_nodes} and "
                ...
                if G.first_pass:
                    for obj in __proto__obj_nodes:
                        ...
                        #     ('arg_paths', orig_arg_paths.union(new_arg_paths)))
                if __proto__obj_nodes:
                    __name_nodes, __obj_nodes, __t = find_prop(
                        G,
                        __proto__obj_nodes,
                        prop_name,
                        branches,
        ...
        if not in_proto and prop_name == wildcard:
            for name_node in G.get_prop_name_nodes(parent_obj):
                ...
                prop_objs = G.get_objs_by_name_node(name_node, branches=branches)
                if prop_objs:
                    prop_obj_nodes.update(prop_objs)
                if G.first_pass:
                    for obj in prop_objs:
        ...
        if (
            (not in_proto or G.check_ipt)
            and prop_name != wildcard
            and (
                not name_node_found
                or G.check_proto_pollution
                or G.check_ipt
                or G.first_pass
            )
        ):
        ...
        if (
            (not in_proto or G.check_ipt)
            and is_wildcard_obj(G, parent_obj)
            ...
            added_obj = G.add_obj_to_name_node(
                added_name_node,
                js_type="object" if G.check_proto_pollution or G.check_ipt else None,
                value=wildcard,
                ast_node=ast_node,
            )
            prop_obj_nodes.add(added_obj)
            ...
            if G.first_pass and prop_name != wildcard:
                G.add_obj_as_prop(
                    prop_name, tobe_added_obj=added_obj, parent_obj=parent_obj
                )
            # propagate tags
            if G.get_node_attr(parent_obj).get("tainted"):
                G.set_node_attr(added_obj, ("tainted", True))
            ...
            if G.get_node_attr(parent_obj).get("unresolved"):
                ...
                G.set_node_attr(
                    ...
                    (
                        "how_to_resolve",
                        G.get_node_attr(parent_obj).get("how_to_resolve"),
                    ),
                )
            for s in prop_name_sources:
                add_contributes_to(G, [s], added_obj)
            ...
            if G.first_pass:
                arg_paths = set(
                    arg_path + "." + str(prop_name) for arg_path in parent_arg_paths
                )
        ...
        # or wildcard property name under normal (known) object
        elif not in_proto and (
            (not name_node_found and prop_name != wildcard)
            ...
            if side == "right":
                ...
                logger.debug(f"Trying to add prop name node under {parent_obj}")
                continue
            else:
...
def handle_prop(
    ...
    if handled_parent.tampered_prop:
        ...
        logger.warning(
            sty.fg.li_red + sty.ef.inverse + "Possible internal"
            " property tampering (parent is tampered) at node {} (Line {})".format(
                ast_node, G.get_node_attr(ast_node).get("lineno:int")
            )
            + sty.rs.all
        )
    ...
    prop_names, prop_name_sources, prop_name_tags = to_values(
        G, handled_prop, for_prop=True
    )
    name_tainted = False
    ...
    if not parent_objs:
        logger.debug(
            "PARENT OBJ {} NOT DEFINED, creating object nodes".format(parent_name)
        )
        # we assume this happens when it's a built-in var name
        if parent_name_nodes:
            parent_objs = []
            for name_node in parent_name_nodes:
                obj = G.add_obj_to_name_node(
                    name_node,
                    ast_node,
                    js_type=(
                        "object" if G.check_proto_pollution or G.check_ipt else None
                    ),
                    value=wildcard,
                )
                parent_objs.append(obj)
        else:
            obj = G.add_obj_to_scope(
                parent_name,
                ast_node,
                js_type="object" if G.check_proto_pollution or G.check_ipt else None,
                scope=G.BASE_SCOPE,
    ...
    for i, prop_name in enumerate(prop_names):
        ...
        name_nodes, obj_nodes, proto_is_tainted = find_prop(
            G,
            parent_objs,
            prop_name,
            branches,
            ...
            ast_node=ast_node,
            prop_name_sources=prop_name_sources[i],
        )
        ...
        if G.check_ipt and side != "left" and proto_is_tainted:
            ...
            logger.warning(
                sty.fg.li_red + sty.ef.inverse + "Possible internal"
                " property tampering (any use) at node {} (Line {})".format(
                    ast_node, G.get_node_attr(ast_node).get("lineno:int")
                )
                + sty.rs.all
            )
    ...
    return (
        NodeHandleResult(
            obj_nodes=list(prop_obj_nodes),
            name=f"{name}",
            name_nodes=list(prop_name_nodes),
            ast_node=ast_node,
            callback=get_df_callback(G),
            name_tainted=name_tainted,
            parent_is_proto=parent_is_proto,
            multi_assign=multi_assign,
            tampered_prop=tampered_prop,
        ),
        handled_parent,
    )
        ...
        if G.get_node_attr(left).get("flags:string[]") == "JS_OBJECT":
            ...
            for child in children:
                ...
                for obj in handled_right.obj_nodes:
                    prop_obj_nodes = G.get_prop_obj_nodes(
                        parent_obj=obj, prop_name=_key, branches=branches
                    )
                    for o in prop_obj_nodes:
                        G.add_obj_as_prop(
                            parent_obj=added_obj, prop_name=_key, tobe_added_obj=o
                        )
                    do_assign(
                        G,
                        handled_left,
                        NodeHandleResult(obj_nodes=prop_obj_nodes),
                        branches,
                        ast_node,
                    )
            ...
            for i, child in enumerate(children):
                ...
                for obj in handled_right.obj_nodes:
                    prop_obj_nodes = G.get_prop_obj_nodes(
                        parent_obj=obj, prop_name=str(i), branches=branches
                    )
                    for o in prop_obj_nodes:
                        G.add_obj_as_prop(
                            parent_obj=added_obj, prop_name=str(i), tobe_added_obj=o
                        )
                    do_assign(
                        G,
                        handled_left,
...
def do_assign(G, handled_left, handled_right, branches=None, ast_node=None):
    ...
    if (
        ...
        and ast_node is not None
        and handled_left.name_tainted
        and handled_left.parent_is_proto
    ):
        flag1 = False
        flag2 = False
        ...
        if flag2:
            name_node_log = [
                ("{}: {}".format(x, repr(G.get_node_attr(x).get("name"))))
                for x in handled_left.name_nodes
            ]
            logger.warning(
                sty.fg.li_red
                + sty.ef.inverse
                + "Possible prototype pollution at node {} (Line {}), "
                "trying to assign {} to name node {}".format(
                    ast_node,
    ...
    if (
        ...
        and any(
            map(
                lambda nn: G.get_node_attr(nn).get("name") == "__proto__",
    ...
    for name_node in handled_left.name_nodes:
        ...
        G.assign_obj_nodes_to_name_node(
            name_node, right_objs, branches=branches, multi=handled_left.multi_assign
        )
    ...
    return NodeHandleResult(
        obj_nodes=handled_right.obj_nodes,
...
def handle_op_simple(G, ast_node, extra=ExtraInfo()):
    ...
    if G.get_node_attr(ast_node).get("type") == "AST_ASSIGN_OP":
        do_assign(
            G,
            handled_left,
            NodeHandleResult(values=results),
            branches=extra.branches,
            ast_node=ast_node,
        )
    return res
...
def handle_op_by_objs(G, ast_node, extra=ExtraInfo(), saved={}):
    ...
    if G.get_node_attr(ast_node).get("type") == "AST_ASSIGN_OP":
        do_assign(
            G,
            handled_left,
            NodeHandleResult(obj_nodes=results),
            branches=extra.branches,
            ast_node=ast_node,
        )
    res = NodeHandleResult(
...
def handle_template(G: Graph, ast_node, extra=ExtraInfo()):
    ...
    if len(children) == 0:
        return NodeHandleResult(
            ast_node=ast_node, values=[""], value_sources=[[ast_node]]
        )
    if len(children) == 1:
...
def handle_var(G: Graph, ast_node, side=None, extra=None):
    ...
    if var_name == "this" and G.cur_objs:
        ...
        return NodeHandleResult(
            name=var_name, values=[G.get_cur_file_path()], ast_node=ast_node
        )
    elif var_name == "__dirname":
        return NodeHandleResult(
            name=var_name,
            values=[os.path.dirname(G.get_cur_file_path())],
            ast_node=ast_node,
        )
    else:
        now_objs = []
        ...
        if name_node is not None:
            now_objs = list(set(G.get_objs_by_name_node(name_node, branches=branches)))
        elif side != "right":
            ...
            if cur_node_attr.get("flags:string[]") == "JS_DECL_VAR":
                ...
                name_node = G.add_name_node(var_name, scope=G.find_ancestor_scope())
...
def handle_node(G: Graph, node_id, extra=None) -> NodeHandleResult:
    ...
    if (
        ...
        and G.func_start_time is not None
    ...
    node_name = cur_node_attr.get("name") or G.get_name_from_child(
        node_id, max_depth=2, order=1
    )
    ...
    try:
        if len(node_code) > 100:
            node_code = ""
    except:
        node_code = ""

    logger.info(
        f"{sty.ef.b}{sty.fg.cyan}HANDLE NODE {node_id}{sty.rs.all}"
    ...
    if cur_type == "File" or cur_type == "Directory":
        ...
        def python_callback(G, caller_ast, extra, _, value=NodeHandleResult(), *args):
        ...
        for promise in promises.obj_nodes:
            then = G.get_prop_obj_nodes(G.promise_prototype, "then", extra.branches)
            result, _ = call_function(
                G,
                then,
        ...
        G.add_edge_if_not_exist(
            exit_node, G.find_nearest_upper_CPG_node(node_id), {"type:TYPE": "FLOWS_TO"}
        )
        return NodeHandleResult(
            ast_node=node_id,
        ...
        for child in children:
            result = handle_node(G, child, ExtraInfo(extra, parent_obj=added_obj))
        ...
        if not (extra and extra.parent_obj is not None):
            ...
            for obj in value_objs:
                G.add_obj_as_prop(
                    key, node_id, parent_obj=extra.parent_obj, tobe_added_obj=obj
                )
        return NodeHandleResult(
            obj_nodes=value_objs, callback=get_df_callback(G)  # used_objs=used_objs,
        )
        ...
        obj_nodes = G.get_func_decl_objs_by_ast_node(
            node_id, scope=G.find_ancestor_scope()
        )
        ...
        if flag == "BINARY_BOOL_OR":
            ...
            now_objs = list(
                set(
                    to_obj_nodes(G, handled_left, node_id)
            ...
            if not d:
                return NodeHandleResult(
                    values=[wildcard], obj_nodes=[G.true_obj, G.false_obj]
                )
            elif p == 1:
        ...
        try:
            if cur_type == "integer" and code.startswith("0x") or code.startswith("0X"):
                ...
                cur_type == "integer" and code.startswith("0b") or code.startswith("0B")
            ):
                value = int(code, 2)
            ...
            logger.error(
                "ValueError for {} at AST node {}, Line {}".format(
        ...
        #     G.solve_from = None
        return NodeHandleResult(
            obj_nodes=r.obj_nodes,
            used_objs=r.used_objs,
            values=r.values,
            value_sources=r.value_sources,
            ast_node=node_id,
            callback=get_df_callback(G),
        )
        ...
        if G.first_pass:
            ...
            possibility, deterministic, tag = check_condition(G, condition, extra)
        if deterministic and possibility == 1:
            ...
            if len(if_elems) == 2:
                ...
                logger.debug(
                    f'Distance from "if" to goal: {if_dist}\n'
                    f'Distance from "else" to goal: {else_dist}'
                )
                if (
                    if_dist is not None
                    and else_dist is not None
        ...
        if deterministic and possibility == 1:
            ...
            if G.first_pass or (len(h1.obj_nodes) <= 100 and len(h2.obj_nodes) <= 100):
                return NodeHandleResult(
                    obj_nodes=h1.obj_nodes + h2.obj_nodes,
                    name_nodes=h1.name_nodes + h2.name_nodes,
                    ast_node=node_id,
                    values=h1.values + h2.values,
                    value_sources=h1.value_sources + h2.value_sources,
                    callback=get_df_callback(G),
        ...
        while True:
            ...
            for name, obj_nodes in d.items():
                logger.debug(
                    ...
                    + ", ".join(
                        [
                            (sty.fg.green + "{}" + sty.rs.all + " {}").format(
                                obj, val_to_str(G.get_node_attr(obj).get("code"))
                            )
            ...
            # avoid infinite loop
            if (
                (not deterministic and counter >= max_count)
                or (counter >= 10 * max_count)
                or check_result == 0
            ):
        ...
        while True:
            ...
            logger.debug(
                "While loop condition {} result: {} {}".format(
            ...
            # avoid infinite loop
            if (
                (not deterministic and counter > G.call_limit)
                or check_result == 0
                or counter > 10
                or (counter >= 1 and G.first_pass)
            ):
        ...
        for v in values:
            ...
            if (
                v != float("nan")
                and G.get_node_attr(node_id).get("flags:string[]") == "UNARY_MINUS"
            ):
                new_values.append(-v)
            else:
                new_values.append(v)
...
def decl_vars_and_funcs(G, ast_node, var=True, func=True):
    ...
    for child in children:
        ...
        if (
            var
            and node_type == "AST_VAR"
            and G.get_node_attr(child)["flags:string[]"] == "JS_DECL_VAR"
        ):
            # var a;
            name = G.get_name_from_child(child)
            if (
                G.get_name_node(name, scope=func_scope, follow_scope_chain=False)
                is None
            ):
                G.add_obj_to_scope(
                    name=name, scope=func_scope, tobe_added_obj=G.undefined_obj
                )
        elif var and node_type == "AST_ASSIGN":
            # var a = ...;
            children = G.get_ordered_ast_child_nodes(child)
            if (
                G.get_node_attr(children[0])["type"] == "AST_VAR"
                and G.get_node_attr(children[0])["flags:string[]"] == "JS_DECL_VAR"
            ):
...
def jsflow_function(G, func_ast, branches=None, block_scope=True, caller_ast=None):
    ...
    if G.two_pass:
        ...
        if G.first_pass:
            ...
            if (
                not G.dont_quit
                and entry_node not in G.possible_cf_nodes
                and file_path
                and not file_path.startswith(builtin_path)
    ...
    if caller_ast is not None:
        if G.caller_counter[caller_ast] >= G.call_limit:
            logger.warning(
                f"{caller_ast}: Function {func_ast} in call stack "
    ...
    logger.info(
        sty.ef.inverse
        + sty.fg.green
        + "FUNCTION {} {} STARTS, SCOPE {}, DECL OBJ {}, this OBJs {}, branches {}".format(
            func_ast,
    ...
    for last_stmt in G.last_stmts:
        G.add_edge_if_not_exist(last_stmt, exit_node, {"type:TYPE": "FLOWS_TO"})
...
def jsflow_block(
    ...
    if branches is None or G.single_branch:
        branches = BranchTagContainer()
    if not G.dont_quit and (
        G.rough_call_dist and G.get_node_attr(ast_node).get("distance_to_goal") is None
    ):
    ...
    for stmt in stmts:
        if G.two_pass and not G.first_pass:
            ...
            if not G.dont_quit and stmt not in G.possible_cf_nodes:
                logger.log(
                    ATTENTION,
                    "Statement {} (Line {}) SKIPPED by rough CF filtering".format(
                        stmt, G.get_node_attr(stmt).get("lineno:int"), G.cur_scope
                    ),
                )
                continue
        ...
        if (
            stmt in G.possible_cf_nodes  # avoid going all the way deep from stdin
            and G.get_node_attr(stmt).get("type") not in ["AST_IF", "AST_SWITCH"]
        ):
...
def call_func_obj(
    ...
    returned_value_sources=None,
    exit_nodes=None,
    fake_returned_objs=None,
    fake_created_objs=None,
    dont_skip=False,
    promise_related=False,
):
    ...
    if G.get_node_attr(func_obj).get("unresolved") == True:
        if (
            G.cur_func_call is None
            or G.cur_func_call[2] is None
            or (G.cur_func_call[2] < call_ast)
        ):
            ...
            if new_vector in reversed(G.stack1):  # avoid duplicated entries
                logger.warning(
                    "Vector {} already in stack 1. Call stack: {}".format(
                        new_vector, G.call_stack
                    )
                )
            else:
            ...
            G.finished = True
            if (
                call_ast is not None
                and G.cur_func_call[2] is not None
    ...
    if func_obj_attrs.get("target_func"):
        ...
        logger.log(
            ATTENTION,
            "Bound function found ({}->{}), this={}".format(
                func_obj_attrs.get("target_func"), func_obj, _this.obj_nodes
            ),
        )
    ...
    if not _this and func_obj_attrs.get("parent_scope_this"):
        _this = NodeHandleResult(obj_nodes=func_obj_attrs.get("parent_scope_this"))
    ...
    if func_obj in G.internal_objs.values():
        logger.warning(
            "Error: Trying to run an internal obj {} {}"
    ...
    if python_func:  # special Python function
        ...
        if is_new:
            if func_obj in G.builtin_constructors:
                logger.log(
                    ATTENTION, f"Running Python function {func_obj} {python_func}..."
                )
                try:
                    h = python_func(
                        G,
                        call_ast,
                        ExtraInfo(extra, branches=next_branches),
                        _this,
                        *_args,
                    )
                ...
                logger.error(
                    f"Error: try to new Python function {func_obj} {python_func}..."
                )
                G.call_stack.pop()
                return [], [], [], False, False
        else:
            ...
            try:
                h = python_func(
                    G, call_ast, ExtraInfo(extra, branches=next_branches), _this, *_args
                )
        ...
        if func_ast:
            ...
            NodeHandleResult(
                ...
                callback=get_df_callback(
                    G, ast_node=dummy_stmt if G.new_trace_rule else None
                ),
            )
            # branch_used_objs = []
        ...
        if G.first_pass and func_ast is not None:
            ...
            if (
                not (file_path and file_path.startswith(builtin_path))
                # not a modeled function
                ...
                if returned_objs is not None and used_objs is not None:
                    ...
                    logger.debug(
                        "saved returned objs {}, created objs {}".format(
                ...
                # elif not dont_skip:
                    ...
                    G.set_node_attr(
                        obj,
                        ("how_to_resolve", (func_obj, func_name, _args, _this, None)),
                    )
        ...
        if (
            G.first_pass
            and flag
            and (G.get_node_attr(func_ast).get("labels:label") != "Artificial_AST")
        ):  # except dummy functions
            add_saved_cf(G, func_ast, call_ast, _args, branches)

        if not flag:  # second pass (fine-grained)
            # if AST cannot be found, create AST
            if func_ast is None or G.get_node_attr(func_ast).get("type") not in [
                "AST_FUNC_DECL",
                "AST_CLOSURE",
                "AST_METHOD",
                "AST_TOPLEVEL",
            ...
            if (
                "labels:label" in func_ast_attr
                and func_ast_attr["labels:label"] == "Artificial_AST"
            ):
                pass
            else:
            ...
            func_scope = G.add_scope(
                new_scope_type,
                func_ast,
                f"Function{func_ast}:{call_ast}",
                func_obj,
                call_ast,
                func_name,
            ...
            param_list = G.get_child_nodes(
                func_ast, edge_type="PARENT_OF", child_type="AST_PARAM_LIST"
            )
            params = G.get_ordered_ast_child_nodes(param_list)
            # add "arguments" array
            arguments_obj = G.add_obj_to_scope(
                name="arguments", js_type="array", scope=func_scope, ast_node=func_ast
            )
            j = 0
                ...
                if j < len(_args):
                    ...
                    for obj in arg_obj_nodes:
                        ...
                        if G.first_pass:
                            ...
                            G.set_node_attr(
                                obj, ("arg_paths", arg_paths)
                            )  # for objs without arg_paths
                    # add argument to the parameter
                    if j < len(params):
                        param = params[j]
                        param_name = G.get_name_from_child(param)
                        if (
                            G.get_node_attr(param).get("flags:string[]")
                            == "PARAM_VARIADIC"
                        ):
                            arr = G.add_obj_to_scope(
                                param_name, call_ast or param, "array", scope=func_scope
                            )
                            length = 0
                            while j < len(_args):
                                logger.debug(
                                    f"add arg {param_name}[{length}]"
                                    f" <- {arg_obj_nodes}, scope {func_scope}"
                                )
                                for obj in arg_obj_nodes:
                                    G.add_obj_as_prop(
                                        str(length), parent_obj=arr, tobe_added_obj=obj
                                    )
                                j += 1
                                length += 1
                            G.add_obj_as_prop(
                                "length", js_type="number", value=length, parent_obj=arr
                            )
                        else:
                            logger.debug(
                                f"add arg {param_name} <- "
                                f"{arg_obj_nodes}, scope {func_scope}"
                            )
                            for obj in arg_obj_nodes:
                                G.add_obj_to_scope(
                    ...
                    # add dummy arguments
                    if G.get_node_attr(param).get("flags:string[]") == "PARAM_VARIADIC":
                        # rest parameter (variable length arguments)
                        added_obj = G.add_obj_to_scope(
                            name=param_name,
                            scope=func_scope,
                            ast_node=call_ast or param,
                            js_type="array",
                        )
                        elem = G.add_obj_as_prop(
                            wildcard,
                            call_ast or param,
                            value=wildcard,
                            parent_obj=added_obj,
                        ...
                        G.get_node_attr(G.get_ordered_ast_child_nodes(param)[2]).get(
                            "type"
                        )
                        == "NULL"
                    ...
                    for added_obj in added_objs:  # Oh! What a big bug!
                        ...
                        if G.first_pass:
                            G.set_node_attr(
                                added_obj, ("arg_paths", {f"{func_scope}:{j}"})
                            )
                        logger.debug(
                            f"add arg {param_name} <- new obj {added_obj}, "
                    ...
                    logger.debug(
                        f"add arguments[{j}] <- new obj {added_obj}, "
                        f"scope {func_scope}, ast node {call_ast}"
                    )
                else:
                    break
                j += 1
            arguments_length_obj = G.add_obj_as_prop(
                prop_name="length", parent_obj=arguments_obj, value=j, js_type="number"
            )
            ...
            if is_new:
                branch_created_obj, branch_returned_objs = instantiate_obj(
                    G, call_ast, func_ast, branches=next_branches
                )
            ...
            if not G.first_pass:
                ...
                for name_node in G.get_prop_name_nodes(arguments_obj):
                    for obj_node in G.get_child_nodes(
        ...
        if G.get_node_attr(func_ast).get("labels:label") == "Artificial_AST":
            ...
            if is_new and branch_created_objs == []:
                if not fake_created_objs:
                    added_obj = G.add_obj_node(
                        ast_node=(
                            dummy_stmt if G.new_trace_rule else call_ast
                        ),  # really?
                        js_type="object",
                        value=wildcard,
                    )
        ...
        if (
            branch_returned_objs is None or branch_used_objs is None
        ):  # workaround for skipping instantiating objects
            func_skipped = True
        # else:
    ...
    if (
        call_ast is not None
        and func_ast is not None
        and G.get_node_attr(func_ast).get("type")
        in ["AST_FUNC_DECL", "AST_CLOSURE", "AST_METHOD"]
    ):
        ...
        if caller_func is not None:
            G.add_edge_if_not_exist(
                caller_func,
                func_ast,
                {"type:TYPE": "CALLS", "promise_related": promise_related},
            )
            logger.info(
                sty.fg.li_magenta
                + sty.ef.inverse
                ...
                + " {} -> {} (Line {} -> Line {}) {} -> {}".format(
                    ...
                    G.get_node_attr(func_ast).get("lineno:int") or "?",
                    G.get_name_from_child(caller_func),
                    func_name,
                )
            )
        else:
            logger.error(
                "Cannot find the caller function for call {}->{}".format(
                    call_ast, func_ast
                )
            )
        ...
        entry_node = G.get_successors(func_ast, edge_type="ENTRY")[0]
        G.add_edge_if_not_exist(caller_cpg, entry_node, {"type:TYPE": "FLOWS_TO"})
        # collect exit nodes
        exit_node = G.get_successors(func_ast, edge_type="EXIT")[0]
...
def save_cf(G, func_obj, call_ast, branches, args=[]):
    ...
    def _find_props(parent_obj, parent_obj_name):
        ...
        """
        nonlocal G, tried
        proto_objs = G.get_prop_obj_nodes(parent_obj=parent_obj, prop_name="__proto__")
        result_objs = []
        result_obj_names = []
        for name_node in G.get_prop_name_nodes(parent_obj):
            name = G.get_node_attr(name_node).get("name")
        ...
        for proto_obj in proto_objs:
            for name_node in G.get_prop_name_nodes(proto_obj):
                ...
                for obj in obj_nodes:
                    ...
                    result_obj_names.append(f"{parent_obj_name}.__proto__.{name}")
    ...
    if func_obj_origin_scope is not None:
        ...
        try:
            func_obj_origin_func = G.get_out_edges(
                func_obj_origin_scope, edge_type="SCOPE_TO_AST"
            )[0][1]
        except IndexError:
            logger.error(
                "Error finding func ast for scope {}".format(func_obj_origin_scope)
            )
        ...
        if func_obj_origin_func is not None and func_obj_access_path is not None:
            saved_calls = G.get_node_attr(func_obj_origin_func).get("saved_calls", [])
            duplicated = False
            ...
            if not duplicated:
                ...
                G.set_node_attr(
                    func_obj_origin_func, ("saved_calls", list(saved_calls))
                )
                logger.info(
                    sty.fg.li_magenta
                    + sty.ef.inverse
                    + "Call {} at arg {} saved to function {} {}".format(
                        call_ast,
                        func_obj_access_path,
                        func_obj_origin_func,
                        G.get_name_from_child(func_obj_origin_func),
                    )
                    + sty.rs.all
                )
                logger.info(
                    sty.fg.li_magenta + "Arugments" + sty.rs.all + " {}".format(args)
                )
...
def add_saved_cf(G, func_ast, call_ast, args, branches):
    ...
    def find_prop_by_path(cur_obj, path, arg_seg, arg_obj):
        ...
        if cur_obj is None:
            ...
            next_objs = G.get_prop_obj_nodes(cur_obj, prop_name=name, branches=branches)
        if len(path) == 1:
            return next_objs
        else:
            l = []
    ...
    #         return l
    def add_additional_paths(
        arg_obj, split_original_path, seg_in_original_path, call_ast, saved_args
    ):
        nonlocal G, func_ast
        ...
        for path in arg_paths:
            ...
            if path in old_path_seg or old_path_seg in path:
                logger.error(
                    "Error: recursive path replace attempt: "
            ...
            logger.info(
                ...
                + "Call {} at alt arg {} saved to function {} {}".format(
                    ...
                    new_path,
                    origin_func_ast,
                    G.get_name_from_child(origin_func_ast),
                )
                + sty.rs.all
            )
            logger.info(
                sty.fg.li_magenta
                + "by replacing {} with {}".format(old_path_seg, path)
                + sty.rs.all
            )
            logger.info(
                sty.fg.li_magenta + "Arugments" + sty.rs.all + " {}".format(saved_args)
            )
                ...
                for arg_obj in to_obj_nodes(G, args[saved_arg_num], call_ast):
                    ...
                    if len(split_path) < 6:
                        add_additional_paths(
                            arg_obj, split_path, seg_num, saved_call_ast, saved_args
                        )
                    # handle solvable paths
                    callee_objs = find_prop_by_path(None, split_path, seg_num, arg_obj)
                    logger.debug(
                        "Found callee objs from arg {} by path #{}: {}".format(
                            arg_obj, saved_access_path, callee_objs
                        )
                    )
                    for callee_obj in callee_objs:
                        callee_ast = G.get_obj_def_ast_node(
                            callee_obj, aim_type="function"
                        )
                        if callee_ast is None:
                            continue
                        if callee_ast == func_ast:
                            logger.error(
                                f"Error: recursive path propagation attempt:"
                        ...
                        if G.get_node_attr(callee_ast).get("type") in [
                            ...
                            callee_entry_node = G.get_successors(
                                callee_ast, edge_type="ENTRY"
                            )[0]
                            G.add_edge_if_not_exist(
                                saved_call_cpg,
                                callee_entry_node,
                            ...
                            callee_exit_node = G.get_successors(
                                callee_ast, edge_type="EXIT"
                            )[0]
                            G.add_edge_if_not_exist(
                                callee_exit_node,
                                saved_call_cpg,
                ...
                if parent_branch:
                    ...
                    G.add_edge(
                        ...
                        v,
                        {
                            ...
                            "branch": BranchTag(parent_branch, mark="A"),
                        },
                    )
                else:
                ...
                if parent_branch:
                    ...
                    if not flag:
                        ...
                        G.add_edge(
                            ...
                            {
                                "type:TYPE": "NAME_TO_OBJ",
                                "branch": BranchTag(parent_branch, mark="D"),
                            },
                        )
                else:
...
def decl_function(
    ...
    if func_name is not None and func_name != "{anon}" and add_to_scope:
        G.add_obj_to_scope(
            name=func_name, scope=obj_parent_scope, tobe_added_obj=added_obj
        )
        name_node = G.get_name_node(
            func_name, scope=obj_parent_scope, follow_scope_chain=False
        )
    ...
    param_list = G.get_child_nodes(
        node_id, edge_type="PARENT_OF", child_type="AST_PARAM_LIST"
    )
    if param_list:
        ...
        length = len(params)
        if length > 0:
            if G.get_node_attr(params[-1]).get("flags:string[]") == "PARAM_VARIADIC":
                length -= 1
        G.add_obj_as_prop("length", node_id, "number", length, added_obj)

    # G.set_node_attr(node_id, ("VISITED", "1"))
    logger.debug(
        f"{sty.ef.b}Declare function{sty.rs.all} {func_name} {node_id}"
...
def run_toplevel_file(G: Graph, node_id):
    ...
    if file_path in G.file_stack:
        logger.info(
            sty.ef.b + "{} in the file stack, skipped.".format(file_path) + sty.rs.all
        )
        return None
    ...
    logger.info(
        sty.fg(173) + sty.ef.inverse + "FILE {} BEGINS".format(file_path) + sty.rs.all
    )

    # add function object and scope
    func_decl_obj = decl_function(
        G,
        node_id,
        func_name=file_path,
        obj_parent_scope=G.BASE_SCOPE,
        scope_parent_scope=G.BASE_SCOPE,
    )
    func_scope = G.add_scope(
        scope_type="FILE_SCOPE",
        decl_ast=node_id,
        scope_name=G.scope_counter.gets(f"File{node_id}"),
        decl_obj=func_decl_obj,
        func_name=file_path,
    ...
    # add module.exports
    added_module_exports = G.add_obj_as_prop(
        "exports", node_id, parent_obj=added_module_obj
    )
    ...
    module_exports_objs = G.get_prop_obj_nodes(
        parent_obj=module_obj, prop_name="exports"
    )
...
def cf_search_complete(G, source_ast, source_name="?", root=None, file_path=None):
    logger.debug(
        sty.fg.li_magenta
        + f"Start CF search from {source_ast} {source_name}"
        + sty.rs.all
    )
    if G.first_pass:
    ...
    def print_func(x):
        ...
        if node_type in ["AST_FUNC_DECL", "AST_CLOSURE", "AST_METHOD"]:
            ...
            if type(func_name) is not str or func_name.startswith("{anon"):
                ...
                if alias:
                    func_name = alias
            return sty.fg.cyan + x + " " + sty.rs.all + str(func_name)
    ...
    for path in cg_paths:
        ...
        for i in range(1, len(path)):
            ...
            output["cf_paths"].append(
                {"from": path[-i - 1], "to": path[-i], "paths": paths_serialized}
            )
            path_debug = []
    ...
    if root is not None:
        ...
        prec_cg_paths = list(
            cg_dfs(G, root, [root], targets=[source_ast], edge_type="RUNS_BEFORE")
        )
    ...
    # Use log directory if available, otherwise use current directory
    cf_paths_file = "./cf_paths.ndjson"
    if G.log_dir:
        cf_paths_file = os.path.join(G.log_dir, "cf_paths.ndjson")
    with open(cf_paths_file, "a") as fp:
...
def handle_require_legacy(G, node_id, extra=None):
    ...
    """
    # handle module name
    arg_list = G.get_ordered_ast_child_nodes(G.get_ordered_ast_child_nodes(node_id)[-1])
    ...
    for module_name in module_names:
        if (
            module_name in modeled_builtin_modules.modeled_modules
            and G.vul_type != "path_traversal"
        ):
            # Python-modeled built-in modules
            # for now mostly fs
            # if it's path_traversal, do not do this
            returned_objs.add(modeled_builtin_modules.get_module(G, module_name))
        else:
            # actual JS modules
            ...
            if module_name and file_path:
                module_exports_objs = get_module_exports(G, file_path)
            # dynamic require (module name is a variable)
            if module_name is None or module_name == wildcard:
                logger.error("{} trying to require unknown package.".format(node_id))
                continue
            ...
            if not module_exports_objs:
                ...
                logger.log(
                    ATTENTION,
                    f"Generating AST on demand for module "
                ...
                G.set_node_attr(start_id, ("module_exports", module_exports_objs))
...
def handle_require(G: Graph, caller_ast, extra, _, module_names):
    ...
    for module_name in module_names:
        if (
            ...
            file_ast_node = None
            if module_name and file_path:
                module_exports_objs, file_ast_node = get_module_exports(G, file_path)
            # dynamic require (module name is a variable)
            if module_name is None or module_name == wildcard:
                logger.error(
                    "{} trying to require an unknown package.".format(caller_ast)
                )
                continue
            ...
            if not module_exports_objs:
                ...
                if not file_path:  # module not found
                    ...
                    module_exports_objs, file_ast_node = get_module_exports(
                        G, file_path
                    )
                ...
                if G.first_pass:
                    if (
                        ...
                        if file_ast_node is not None:
                            # rerun the file
                            logger.info(
                                "module.exports {} is unresolved, rerun the file...".format(
                                    module_exports_objs[0]
                                )
                            )
                            G.file_stack.append(G.get_node_file_path(file_ast_node))
                            empty_rough_stacks(
                                G, reason="handle_require"
                            )  # don't use this here
                            G.file_stack.pop()
                            _module_exports_objs = module_exports_objs
                            module_exports_objs = run_toplevel_file(G, file_ast_node)
                            unresolved_solved = (
                                G.get_node_attr(module_exports_objs[0]).get(
                                    "unresolved"
                                )
                                is None
                            )
                            logger.info(
                                "old module.exports objs (unresolved) for {}: {}".format(
                                    caller_ast, _module_exports_objs
                                )
                            )
                            logger.info(
                                "new module.exports objs after rerunning: {} {}".format(
                                    module_exports_objs, unresolved_solved
                                )
                            )
                        else:
                            logger.error(
                                "Error: module.exports {} is unresolved but file AST node is unknown".format(
                                    module_exports_objs[0]
                                )
                            )
                if (
    ...
    #         G.finished = True
    return NodeHandleResult(obj_nodes=returned_objs, used_objs=list(chain(*src)))
    ...
    for node in toplevel_nodes:
        ...
        if G.get_node_attr(node).get("name") == file_path:
            ...
            if saved_module_exports != None:
                ...
                logger.log(
                    ATTENTION,
                    "File has been required, "
                    "return saved module.exports {} for {}".format(
                        module_exports_objs, file_path
                    ),
                )
                break
            else:
...
def run_exported_functions(G, module_exports_objs, extra, file_path):
    ...
    if (
        G.two_pass
        and not G.first_pass
        and len(G.file_stack) > 1
        and not G.no_file_based
        and file_path not in G.vul_files
    ):
            ...
            if proto_obj == G.promise_prototype:
                ...
                if executors:
                    for executor in executors:
                        if executor in tried:
                            continue
                        result_objs.append((parent_obj, executor))
                        result_obj_names.append(f"{parent_obj_name}.then()")
        return result_objs, result_obj_names
    ...
    while len(exported_objs) != 0:  # BFS
        ...
        if G.func_entry_point is not None and not (
            G.get_node_attr(obj).get("type") == "function"
            and (
        ...
        if G.first_pass and G.get_node_attr(obj).get("unresolved") == True:
            ...
            returned_objs, created_objs = call_func_obj(
                G, r[0], r[2], r[3], func_name=r[1]
            )[:2]
        ...
        if parent_obj is None:
            ...
            returned_result = NodeHandleResult(
                obj_nodes=list(returned_objs),
                used_objs=list(used_objs),
            ...
            returned_objs, newed_objs, used_objs, func_run, func_skipped = (
                call_func_obj(
                    G,
                    obj,
                    _this=NodeHandleResult(obj_nodes=[parent_obj]),
                    extra=extra,
                    mark_fake_args=True,
                    dont_skip=True,
                )
        ...
        if returned_result is not None:
            ...
            prev_exit_nodes.extend([cur_exit_node] * len(returned_result.obj_nodes))
            roots.extend([cur_root or cur_func_ast] * len(returned_result.obj_nodes))
        ...
        params = G.get_ordered_ast_child_nodes(param_list)
        arg_names = filter(
            lambda x: x is not None, (G.get_name_from_child(param) for param in params)
        )
        args = ",".join(arg_names)
        ...
        if G.check_proto_pollution:
            ...
            if pps:  # if there are new results
                ...
                with open(vul_func_file, "a") as fp:
                    logger.log(
                        ATTENTION,
                        f"proto_pollution found in {G.entry_file_path} at {cur_obj_name}({args})",
                    )
                    fp.write(
                        f'proto_pollution,"{G.entry_file_path}","{cur_obj_name}","{args}",{len(pps)}\n'
                    )
        ...
        if G.check_ipt:
            ...
            if ipts:  # if there are new results
                ...
                with open(vul_func_file, "a") as fp:
                    logger.log(
                        ATTENTION,
                        f"int_prop_tampering found in {G.entry_file_path} at {cur_obj_name}({args})",
                    )
                    fp.write(
                        f'int_prop_tampering,"{G.entry_file_path}","{cur_obj_name}","{args}",{len(ipts)}\n'
                    )
...
def cf_dfs(
    ...
    next_nodes = list(
        dict.fromkeys(map(lambda e: e[1], cf_edges))  # avoid duplicate edges
    )
    func_next_nodes = []  # go to called functions first
    other_next_nodes = []
    for node in next_nodes:
        if node not in path or G.get_node_attr(current).get("type") == "CFG_FUNC_EXIT":
    ...
    if func_next_nodes:
        ...
        yield from cf_dfs(
            G, node, path + [node], targets, reachable, finished, nop, cg_nodes
        )
    else:
...
def cg_dfs(
    ...
    next_nodes = list(
        dict.fromkeys(map(lambda e: e[1], call_edges))  # avoid duplicate edges
    )
    for node in next_nodes:
        if node in path:
            continue
        yield from cg_dfs(
            G, node, path + [node], targets, current, reachable, finished, edge_type
        )
...
def cf_dfs_inside(
    G: Graph, current, path=[], target=None, reachable=None, finished=None, nop=None
):
    if reachable is None:
        reachable = set()
        finished = set()
...
def cgg_cf_dfs(
    ...
    for node in next_nodes:
        ...
        if node not in cg_path_nodes:
            continue
        yield from cg_dfs(
            G, node, cg_path_nodes, path + [node], current, targets, nop, output_nodes
        )
    if caller is not None:  # go back to caller
        yield from cg_dfs(
            G, caller, cg_path_nodes, path + [caller], None, targets, nop, output_nodes
        )
    # if not next_nodes:
    #     yield path # output unreachable paths, for debugging only
...
def handle_call(G, ast_node, extra):
        ...
        if handled_args:
            for obj in handled_args[0].obj_nodes:
                ...
                # wildcard or fixed "object"?
                    ...
                    G.get_node_attr(obj).get("type") == "object"
                    and G.get_node_attr(obj).get("code") == wildcard
                ):
    ...
    func_decl_objs = list(
        filter(
            lambda x: x != G.undefined_obj and x != G.null_obj, handled_callee.obj_nodes
        )
    )
    ...
    if G.get_node_attr(ast_node).get("type") == "AST_CALL":
        ...
        stmt_id = None
    returned_result, created_objs = call_function(
        G,
        func_decl_objs,
        handled_args,
        handled_parent,
...
def call_function(
    ...
    if (G.auto_exploit and not G.first_pass) and (
        ...
        for payload in attack_dict.get(G.vul_type):
            ...
            logger.info(
                sty.ef.b
                + "[AUTO] Constraint solver is solving for {}...".format(
                    G.cur_source_name
                )
                + sty.rs.all
            )
            ...
            for i, (assertions, results) in enumerate(solution):
                ...
                if results:
                    ...
                    if success:
                        ...
                        logger.warning(
                            f"[AUTO] Solving may have failed. Please check logs."
                        )
...
def handle_eval(G: Graph, caller_ast, extra, _, strings, *args):
    ...
    for i, value in enumerate(strings.values):
        if type(value) == str and value != wildcard:
            if not G.single_branch:
                branches = branches + [
                    BranchTag(point=f"Eval{caller_ast}", branch=num_of_branches)
                ]
                num_of_branches += 1
    ...
    for obj in strings.obj_nodes:
        ...
        if value == wildcard or G.get_node_attr(obj).get("type") == "string":
            ...
            if not G.single_branch:
                ...
                num_of_branches += 1
            if value == wildcard:
                result = NodeHandleResult(values=[wildcard], value_sources=[obj])
            else:
                result = js_eval(G, value, ExtraInfo(branches=branches))
            returned_values.extend(result.values)
...
def build_df_by_def_use(G, cur_stmt, used_objs):
    ...
    for obj in used_objs:
        ...
        if def_cpg_node is None:
            continue
        if def_cpg_node == cur_stmt:
            continue
        ...
        logger.info(
            ...
            + "OBJ REACHES"
            + sty.rs.all
            + " {} -> {} (Line {} -> Line {}), by OBJ {}".format(
                def_cpg_node, cur_stmt, def_lineno, cur_lineno, obj
            )
        )
...
def print_handle_result_colored(G: Graph, handle_result: NodeHandleResult):
    ...
    output = (
        f"{sty.ef.b}{sty.fg.cyan}{handle_result.ast_node}{sty.rs.all} "
...
def generate_obj_graph(G: Graph, entry_nodeid):
    ...
    logger.info(
        sty.fg.green + "GENERATE OBJECT GRAPH" + sty.rs.all + ": " + entry_nodeid
    )
...
def analyze_files(G, path, start_node_id=0, check_signatures=[]):
    ...
    Args:
        ...
        check_signatures (list, optional): List of function signatures to check
            for presence. If specified and functions are not found, analysis
            may be skipped. Defaults to empty list.

    Returns:
        int: Return code indicating analysis status:
            - -1: Signature check failed (required functions not found)
            - -2: Two-pass mode but no control flow paths found
    ...
    if G.two_pass or G.coarse_only:
        ...
        logger.info(
            sty.fg.green
            + sty.ef.b
            + "1st run finished in {:.2f}s. ".format(time.time() - time1)
            + sty.rs.all
        )
    ...
    if G.two_pass:
        ...
        cf_edges = list(
            filter(
                lambda e: int(e[0]) < G.num_of_ast_nodes
                and int(e[1]) < G.num_of_ast_nodes,
                cf_edges,
            )
        )
        ...
        logger.info(sty.fg.green + sty.ef.b + "Starting 2nd run..." + sty.rs.all)
    if not G.coarse_only:
        ...
        logger.info(
            sty.fg.green
            + sty.ef.b
            + "2nd run finished in {:.2f}s. ".format(time.time() - time2)
            + sty.rs.all
        )
    ...
    if generate_graph:
        if G.two_pass or G.coarse_only:
            ...
            logger.info(
                sty.fg.green
                + sty.ef.b
                + "1st run finished {:.2f}s. ".format(time.time() - time1)
                + sty.rs.all
            )
            first_pass_time = time.time() - time1
        if G.two_pass:
...
def log_cf(G, first_pass_time, second_pass_time):
    ...
    # Create/update eval_logger to use log directory if available
    global eval_logger
    if G.log_dir:
        eval_file = os.path.join(G.log_dir, "evaluation.ndjson")
    else:
        eval_file = "evaluation.ndjson"
    eval_logger = create_logger("eval_logger", output_type="file", file_name=eval_file)
    eval_logger.info(
        json.dumps(
            {
                "file": G.entry_file_path,
                "type": "time",
                "pass": "1",
        ...
        json.dumps(
            {
                ...
                "type": "time",
                "pass": "1+2",
                "time": first_pass_time + second_pass_time,
                "dynamic_calls": len(G.dynamic_calls),
                "total_calls": len(G.total_calls),
                "ast_nodes": G.num_of_ast_nodes,
                "loc": loc,
            }
        )
    )
        ...
        json.dumps(
            {
                ...
                "type": "time",
                "pass": "1+2+3",
                "time": first_pass_time + second_pass_time + G.solver_time,
                "dynamic_calls": len(G.dynamic_calls),
                "total_calls": len(G.total_calls),
                "ast_nodes": G.num_of_ast_nodes,
                "loc": loc,
        ...
        json.dumps(
            {
                ...
                "cf": [
                    ...
                    len(call_edges),
                    real_ce_counter,
                    len(cf_edges) - len(call_edges),
                    real_cf_counter - real_ce_counter,
                    promise_related_ce_counter,
                ],
                "calls": [
                    len(G.dynamic_calls),
                    len(G.static_calls),
                    len(G.unresolvable_calls),
                    len(G.total_calls),
                ],
            }
        )
...
def handle_var_prop_name(G: Graph, exp, extra=ExtraInfo()) -> NodeHandleResult:
    if "." in exp:
        ...
        if prop_name == "*":
            prop_name = wildcard
        parent_objs = list(
            filter(lambda x: x != G.undefined_obj, handled_parent.obj_nodes)
        )
        if not parent_objs:
            ...
            # find property name nodes and object nodes
            name_nodes, obj_nodes = find_prop(
                G, parent_objs, prop_name, extra.branches, parent_name
            )
            name_nodes, obj_nodes = list(name_nodes), list(obj_nodes)
            return NodeHandleResult(
                obj_nodes=obj_nodes, name_nodes=name_nodes, name=exp
            )
    else:
        ...
        if name_node is None:
            ...
            obj_nodes = list(
                set(G.get_objs_by_name_node(name_node, branches=extra.branches))
            )
            return NodeHandleResult(
                obj_nodes=obj_nodes, name_nodes=[name_node], name=exp
            )
...
def find_blocks(G: Graph, ast_node):
    ...
    if node_type in ["AST_FUNC_DECL", "AST_CLOSURE", "AST_METHOD", "AST_STMT_LIST"]:
        yield ast_node
...
def find_function(G: Graph, ast_node):
    ...
    """
    node_type = G.get_node_attr(ast_node).get("type")
    if node_type in ["AST_FUNC_DECL", "AST_CLOSURE", "AST_METHOD", "AST_TOPLEVEL"]:
        return ast_node
    else:
        for e in G.get_in_edges(ast_node, edge_type="PARENT_OF"):
...
def build_rough_call_graph(G: Graph, entry_node):
    ...
    def get_func_name(G: Graph, ast_node, recursive=True):
        ...
        if node_type in ["AST_FUNC_DECL", "AST_CLOSURE", "AST_METHOD", "AST_STMT_LIST"]:
            ...
            return get_func_name(
                G, G.get_ordered_ast_child_nodes(ast_node)[0], recursive=False
            )
    ...
    logger.debug(
        sty.fg.magenta + sty.ef.b + "Building rough call graph..." + sty.rs.all
    )
    ...
    while q:
        ...
        callers = [e[0] for e in G.get_in_edges(callee, edge_type="POSSIBLY_CALLS")]
...
def handle_for_in(G: Graph, ast_node, extra):
    ...
    parent_scope = G.cur_scope
    G.cur_scope = G.add_scope(
        "BLOCK_SCOPE", decl_ast=body, scope_name=G.scope_counter.gets(f"Block{body}")
    )
    G.scope_stack.append(G.cur_scope)
    ...
    for obj in handled_obj.obj_nodes:
        ...
        for k in prop_names:
            ...
            # print(G.for_stack)
            G.assign_obj_nodes_to_name_node(
                handled_key.name_nodes[0], [key_obj], branches=extra.branches
            )
            # run the body
            G.last_stmts = [ast_node]
...
def handle_for_in_fast(G: Graph, ast_node, extra):
    ...
    G.assign_obj_nodes_to_name_node(
        handled_key.name_nodes[0], key_objs, branches=extra.branches
    )
    # run the body
    G.last_stmts = [ast_node]
    jsflow_block(G, body, branches=extra.branches)
...
def handle_for_of(G: Graph, ast_node, extra):
    ...
    for obj in handled_obj.obj_nodes:
        ...
        # if it's a wildcard object, include "__proto__"
        prop_name_nodes = G.get_prop_name_nodes(
            obj,
            exclude_proto=not is_wildcard_obj(G, obj),
            numeric_only=numeric_only,
            exclude_wildcard=True,
        ...
        if is_wildcard_obj(G, obj):
            ...
            if not wildcard_prop_obj_nodes or not wildcard_prop_obj_nodes:
                wildcard_prop_obj_nodes = [
                    G.add_obj_as_prop(
                        wildcard, ast_node, value=wildcard, parent_obj=obj
                    )
                ]
                wildcard_prop_name_node = G.get_prop_name_node(wildcard, obj)
        ...
        prop_obj_nodes = list(
            map(
                lambda nn: G.get_obj_nodes(nn, branches=extra.branches), prop_name_nodes
            )
        )
        for name_node, obj_nodes in zip(prop_name_nodes, prop_obj_nodes):
            ...
            # print(G.for_stack)
            G.assign_obj_nodes_to_name_node(
                handled_value.name_nodes[0], obj_nodes, branches=extra.branches
            )
            # run the body
            G.last_stmts = [ast_node]
            jsflow_block(G, body, branches=extra.branches)
...
def handle_for_of_fast(G: Graph, ast_node, extra):
    ...
    for obj in handled_obj.obj_nodes:
        ...
        # if it's a wildcard object, include "__proto__"
        prop_obj_nodes = G.get_prop_obj_nodes(
            obj, exclude_proto=not is_wildcard_obj(G, obj), numeric_only=numeric_only
        )
        prop_objs.update(prop_obj_nodes)
    G.assign_obj_nodes_to_name_node(
        handled_value.name_nodes[0], prop_objs, branches=extra.branches
    )
    # run the body
    G.last_stmts = [ast_node]
    jsflow_block(G, body, branches=extra.branches)
...
def handle_for_of_legacy(G: Graph, ast_node, extra):
    ...
    for obj in handled_obj.obj_nodes:
        ...
        if is_wildcard_obj(G, obj):
            # wildcard property for wildcard object
            wildcard_prop_obj_nodes = G.get_prop_obj_nodes(
                obj, prop_name=wildcard, branches=extra.branches
            )
        ...
        G.for_stack.append(
            ("for-of", ast_node, G.cur_scope, None, [], handled_obj.obj_nodes)  # TODO
        )
        # print(G.for_stack)
        # assign the object node to the loop variable
        G.assign_obj_nodes_to_name_node(
            handled_value.name_nodes[0], prop_obj_nodes, branches=extra.branches
        )
        # run the body
        G.last_stmts = [ast_node]
        jsflow_block(G, body, branches=extra.branches)
...
def handle_class(G: Graph, ast_node, extra):
    ...
    toplevel = children[4]
    body = G.get_child_nodes(
        toplevel, edge_type="PARENT_OF", child_type="AST_STMT_LIST"
    )[0]
    prev_dont_quit = G.dont_quit
    G.dont_quit = "class"
...
def handle_method(G: Graph, ast_node, extra):
    ...
    if name == "constructor":
        ...
        prototypes = G.get_prop_obj_nodes(
            extra.class_obj, "prototype", branches=extra.branches
        )
        for p in prototypes:
...
def empty_rough_stacks(G, reason=None):
    logger.info(
        sty.fg.magenta
        + sty.ef.inverse
        + "Start to empty stack1 {} and stack2 {}".format(len(G.stack1), len(G.stack2))
        + sty.rs.all
    )
    cur_file_path = G.file_stack[-1] if G.file_stack else None
    G.finished = False
    while True:
        ...
        while True:
            ...
            if G.stack2:
                ...
                if (
                    s2i[5] is not None
                    and cur_file_path is not None
                    and s2i[5] != cur_file_path
                ):
                    ...
                    logger.debug(
                        sty.fg.magenta
                        + "Break from stack2 {}\nCurrent file path {}\nAll cleared {}".format(
                            G.stack2, cur_file_path, all_cleared
                        )
                        + sty.rs.all
                    )
                    break
        ...
        if G.stack1:
            ...
            if (
                ...
                logger.debug(
                    sty.fg.magenta
                    + "Break from stack1 {}\nCurrent file path {}\nAll cleared {}".format(
                        G.stack1, cur_file_path, all_cleared
                    )
                    + sty.rs.all
                )
                empty_task_queues(G)
